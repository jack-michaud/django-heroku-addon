...
from api.models import Guple, GupleStore, Plan
from django_heroku_addon.settings import *
import logging
import json
import base64
import hashlib
...
def heroku_resources(request, id):
	if request.method == "DELETE":
		return heroku_deprovision(request, id)

	elif request.method == "PUT":
		return heroku_planchange(request, id)
...
def heroku_planchange(request, id):
	...
	guple_store = GupleStore.objects.get(id=id)
	logger.info("Plan change to %s" % _json_body(request)['plan'])
